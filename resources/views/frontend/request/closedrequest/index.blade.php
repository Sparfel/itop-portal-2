...
@section('content_header')
    <h1><i class="fas fa-lock"></i> {{ __('Your Closed Requests') }}
        @php
            ...
            $resolution_code = request('resolution_code', 'all');
            $resolutioncodeIcon = $resolution_code !== 'all' ? getResolutioncodeIcon($resolution_code) : '';
            $resolutioncodeClass = $resolution_code !== 'all' ? getResolutioncodeColor($resolution_code, true) : 'bg-info';
            $resolutioncodeLabel = $resolution_code !== 'all' ? ucfirst($resolution_code) : 'Tous';
        @endphp

            <!-- Affichage du type de requête si sélectionné -->
        @if($type !== 'all')
            <a href="{{ route('closedrequest.filter', ['priority' => request('priority', 'all'), 'resolution_code' => request('resolution_code', 'all'), 'request_type' => 'all']) }}">
        ...
        @if($priority !== 'all')
            <a href="{{ route('closedrequest.filter', ['priority' => 'all', 'resolution_code' => request('resolution_code', 'all'), 'request_type' => request('request_type', 'all')]) }}">
        ...
        @if($resolution_code !== 'all')
            <a href="{{ route('closedrequest.filter', ['priority' => request('priority', 'all'), 'request_type' => request('request_type', 'all'), 'resolution_code' => 'all']) }}">
        <span class="badge {{ $resolutioncodeClass }}">
            <i class="{{ $resolutioncodeIcon }}"></i> {{ $resolutioncodeLabel }} &times;
        </span>
            </a>&nbsp;
        @endif
        ...
        <div class="container-fluid text-sm">
            <div class="row">
                ...
                <div class="col-md-9">
                    ...
                    @include('frontend.request.closedrequest._type-ticket')
...
{{--                            </div>--}}
                ...
                <div class="col-md-3">
                    ...
                    @include('frontend.request.closedrequest._resolutioncode-widget')
                    @include('frontend.request.closedrequest._priority-ticket')
                </div>
            </div>
        </div>
    </section>
